...
| CA-7 | Yes | n/a | Per-run completeness enforcement: every expected collector must report in the load, not continuous monitoring or trending over time (the warehouse retains one run) |
| AU-9 / AU-12(3) | Yes | n/a | Evidence integrity: `run_id` stamping ties every row to its collection; raw↔staged reconciliation fails the build when landed and staged row counts diverge |
